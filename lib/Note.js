(function() {
  ...
  Note = (function() {
    ...
    Note.prototype.loadENMLContent = function(note_content) {
      ...
      note_content = note_content.replace(/<\?xml[^>]*>\s*/g, '').replace(/<!(--)?\[CDATA[^>]*>\s*/g, '').replace(/<!DOCTYPE[^>]*>\s*/g, '').replace(/<en-note[^>]*>\s*/g, '').replace(/<\/en-note>\s*/g, '').replace(/]](--)?>\s*/g, '').replace(/<en-media type="image\/([^"]+)" hash="(\w+)"\/>/g, '<img alt="$1 image" src="resources/$2.$1"/>').replace(/<en-media type="(\w+)\/([^"]+)" .*hash="(\w+)"\/>/g, function(match, p1, p2, hash) {
        var f;
        return "<a href=\"resources/" + hash + "/" + (f = note.attachments[hash].fileName) + "\">" + f + "</a>";
      });
