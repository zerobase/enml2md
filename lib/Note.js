(function() {
  ...
  Note = (function() {
    ...
    Note.prototype.loadENMLContent = function(note_content) {
      ...
      note_content = note_content.replace(/<\?xml[^>]*>\s*/g, '').replace(/<!(--)?\[CDATA[^>]*>\s*/g, '').replace(/<!DOCTYPE[^>]*>\s*/g, '').replace(/<en-note[^>]*>\s*/g, '').replace(/<\/en-note>\s*/g, '').replace(/]](--)?>\s*/g, '').replace(/<en-media[^>]*?type="(\w+)\/([^"]+)"[^>]*?hash="(\w+)"[^>]*?(\/>|><\/en-media>)?/g, function(match, type, subtype, hash) {
        var f;
