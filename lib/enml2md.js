...
function parseENMLContent(html) { // return a string (Markdown format).
  var en_note_content = html
    .replace(/<!--\[CDATA[^>]*>\s*/, '')
    .replace(/<!DOCTYPE[^>]*>\s*/, '')
    .replace(/<en-note[^>]*>\s*/, '')
    .replace(/<\/en-note>\s*/, '')
    .replace(/]]-->\s*/, '');
  return html2markdown(en_note_content);
}
